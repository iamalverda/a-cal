...
class GoogleCalendarProvider(CalendarProvider):
    ...
    async def create_event(self, event: CalendarEventDTO) -> CalendarEventDTO:
        created = await self._svc.create_event(self._from_dto(event)) if hasattr(self._svc, "create_event") else event
        return event
    ...
    @staticmethod
    def _to_dto(raw: Dict[str, Any], calendar_id: Optional[str]) -> CalendarEventDTO:
